#!/bin/bash
# GLM-5.3-Flash-FP8 EP8 DISAGG DECODE leg (kv_consumer). Runs INSIDE nite on 038.
# Decode: PIECEWISE cudagraph + MTP(1) + --max-num-seqs 64.
# proxy_ip = PREFILL node 030 = 10.158.215.107 ; proxy_ping_port=36367 (toy proxy hardcoded)
set -u
M=/models/GLM-5.3-Flash-FP8
LOG=/opt/vllm_cache/disagg_decode.log
...
#     ceiling, and disable prefix caching. Coupled knobs: higher MML shrinks the
#     per-pass ceiling; lower MNBT keeps each pass under it. MEASURED EP8/EP8
#     NIAH (needle 74923, 3x): MML=270000 MNBT=16384 -> 64K+100K+256K PASS 3/3.
#     Env-overridable to re-sweep. ---
MML="${MML:-270000}"
MNBT="${MNBT:-16384}"

KV='{"kv_connector":"MoRIIOConnector","kv_role":"kv_consumer","kv_port":"6301","kv_connector_extra_config":{"proxy_ip":"10.158.215.107","proxy_port":"10001","proxy_ping_port":"36367","http_port":"40005","local_ping_port":"4583","handshake_port":"7305","notify_port":"61005","read_mode":"true"}}'

echo "=== EP8 DISAGG DECODE BASELINE (PIECEWISE, no MTP) $(date) ===" > $LOG
# Kill any prior vllm and WAIT for VRAM release (zombies hold HBM; pkill races
# the next launch). Poll until GPU0 used < 5 GiB or 60s elapse.
