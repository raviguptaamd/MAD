...
def get_layer_transfer_geometry(
    ...
    if len(shape) == 4 and shape[1] == 1:
        # GLM53_FLASH_PACKED_4D_FALLBACK: single-head-slot packed 4-D cache fallback for
        # hybrid / packed caches whose physical [B, H=1, N, C] layout the
        # standardized branch above rejects. Occurs in GLM-5.3-Flash / DSA
        # on ROCm (sparse-indexer k_cache; linear-attention recurrent-state
        # cache, e.g. shape (N,1,1,1085440) padded to the attention page).
        # One contiguous (optionally padded) region per block: block_len is
        # the real per-block content while block_stride keeps the padded
        # stride[0] so per-block offsets and the registered span skip the
        # inter-block padding. Byte-identical to the standardized branch for
        # an unpadded spec-matching MLA tensor -> a safe superset.
        num_blocks = shape[0]
...
def _kernel_blocks_per_group_block(num_kernel_blocks, ref_group_blocks) -> int:
    """Kernel-blocks-per-group-block (kbpb) for a single-region MLA/hybrid layer.

    GLM53_INDEXER_KBPB: vLLM's hybrid KV allocator hands out block-ids in the
    *group* block unit (all cache groups share one group-block count, e.g.
    GLM-5.3-Flash decode=1012, prefill=1139). Most layers are paged 1:1 at that
    unit, but the DSA sparse-indexer k_cache is paged at a finer kernel block
    (shape (17204,1,64,132) on decode = 1012*17), so ONE group block-id N spans
    kbpb=17 contiguous kernel sub-blocks [N*kbpb .. N*kbpb+kbpb-1]. Native
    transferred the raw group id as a kernel id, so every group block beyond #0
    (>4352 tok ~= the 4096 recall wall) read the indexer KV from the wrong
    kernel offset -> corruption. Returns kbpb from the num_blocks ratio; 1 (no
    expansion, byte-identical old path) when it doesn't divide cleanly.
    """
    try:
        if (
            ref_group_blocks
            and ref_group_blocks > 0
            and num_kernel_blocks
...
def compute_block_transfer_offsets(
    ...
    import os as _osd
    if _osd.environ.get("MORIIO_OFFSET_DBG") == "1":
        print(
            "[OFFDBG] L={} mla={} shape={} nb={} bs={} bstride={} pb={} "
            "lref={} rnb={} rref={} nloc={} loc12={}".format(
                layer_name, is_mla_cache_layer(layer_to_spec, layer_name),
                tuple(kv_cache.shape), geometry.num_blocks, geometry.block_size,
                geometry.block_stride, per_block, local_num_blocks,
                remote_num_blocks, remote_ref_blocks,
                len(local_block_ids), list(local_block_ids)[:12],
            ),
            flush=True,
        )

    # GLM53_INDEXER_KBPB: expand group block-ids into their kernel sub-blocks for
    # finely-paged single-region MLA layers (the DSA sparse-indexer k_cache).
    # local and remote can have different kernel-page counts (decode 17204 vs
    # prefill 19363) but the SAME group-block-count ratio (17), so per-side
    # expansion keeps the paired lists aligned. Only fires for pb==1
    # single-region layers; kbpb==1 (all other layers) is a no-op that leaves
    # the id lists and every downstream offset byte-identical.
    #   local kbpb  = geometry.num_blocks (LOCAL kernel blocks) / local ref
    #   remote kbpb = remote_num_blocks   (REMOTE kernel blocks) / remote ref
    if per_block == 1:
        local_kbpb = _kernel_blocks_per_group_block(
            geometry.num_blocks, local_num_blocks
        )
        remote_kbpb = _kernel_blocks_per_group_block(
            remote_num_blocks, remote_ref_blocks
        )
        if local_kbpb > 1 or remote_kbpb > 1:
            import os as _os
            if _os.environ.get("MORIIO_OFFSET_DBG") == "1":
                print(
                    "[KBPB] L={} lkbpb={} rkbpb={} nb={} lref={} rnb={} rref={} "
                    "loc->{} rem->{}".format(
                        layer_name, local_kbpb, remote_kbpb, geometry.num_blocks,
                        local_num_blocks, remote_num_blocks, remote_ref_blocks,
                        [lb * local_kbpb + j for lb in local_block_ids[:2]
                         for j in range(local_kbpb)][:6],
                        [rb * remote_kbpb + j for rb in remote_block_ids[:2]
                         for j in range(remote_kbpb)][:6],
                    ),
                    flush=True,
                )
            local_block_ids = [
                lb * local_kbpb + j
                for lb in local_block_ids
                for j in range(local_kbpb)
            ]
            remote_block_ids = [
                rb * remote_kbpb + j
                for rb in remote_block_ids
                for j in range(remote_kbpb)
            ]
