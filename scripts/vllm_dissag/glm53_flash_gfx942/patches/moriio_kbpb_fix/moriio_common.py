...
class MoRIIOAgentMetadata(
    ...
    # GLM53_PERGROUP_REMOTE_BLOCKS: per-layer remote block counts (hybrid models have
    # different block counts per KV group; the scalar num_blocks above is
    # only correct for the first layer). Optional -> old peers omit it.
...
class MoRIIOConfig:
    ...
    def from_vllm_config(cls, vllm_config: VllmConfig) -> "MoRIIOConfig":
        ...
        # tp_size REQUIRED (not the default 1): offset = dp_rank*tp_size + tp_rank.
        # Omitting it collapses to dp_rank+tp_rank -> co-located DP ranks collide on
        # ZMQ ports at TP4xDP2 (harmless at EP8 tp_size=1, FATAL at DP>=2 with tp>1).
