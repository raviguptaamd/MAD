...
class MoRIIOConnectorScheduler:
    ...
    def __init__(
        ...
        if self._is_hma_required:
            ...
            if unsupported:
                # GLM53_RELAX_HMA_GUARD: GLM-5.3-Flash is a genuine MLA + KDA/Mamba
                # hybrid. The per-group KV routing patch (GLM53_HYBRID_KV_
                # NONUNIFORM) handles the non-uniform block layout, so the
                # strict upstream raise is over-conservative here. Warn and
                # proceed (matches the MI355X in-source relaxed HMA guard).
                logger.warning(
                    "MoRIIO HMA: proceeding with non-sliding-window hybrid "
                    "KV groups %s (GLM53 relaxed guard).", sorted(set(unsupported))
                )
    ...
    def _release_write_prefill_blocks(self, request_id: ReqId, params: dict[str, Any]):
        ...
        for tp_index in range(self.tp_size):
            target_port = remote_notify_port + get_port_offset(remote_dp_rank, tp_index, self.tp_size)
                ...
                if remote_block_ids := params.get("remote_block_ids"):
                    ...
                    if "remote_engine_id" in params:
                        if num_external_tokens > 0:
                            ...
                            # GLM53_MTP_DISAGG_RECONCILE
                            # ---------------------------
                            # local_block_ids / remote_block_ids are
                            # LISTS-OF-LISTS: one inner list per KV cache
                            # group. GLM-5.3-Flash is a hybrid-KV model
                            # (MLA main latent + DSA sparse-indexer k_cache +
                            # KpoolTail sliding window + KDA/Mamba state), so
                            # the no-MTP legs both carry 4 groups.
                            #
                            # With MTP speculative decode (--speculative-config
                            # method=mtp) the DECODE leg instantiates
                            # Glm5NextMTPModel: its draft layer (hf layer index
                            # == num_hidden_layers, built AFTER the main model)
                            # has its own attention whose KV spec does not merge
                            # into any base group, so it forms EXTRA trailing KV
                            # group(s). The prefill leg has NO MTP and never
                            # computes that KV (MTP's KV is produced locally at
                            # decode time), so remote_block_ids has FEWER groups
                            # than local_block_ids. The original
                            # zip(..., strict=True) then raised ValueError and
                            # killed the decode EngineCore on the first request.
                            #
                            # Reconcile: the base groups are registered
                            # identically on both legs and occupy the SAME
                            # leading indices [0, M); the MTP-only group(s) are
                            # appended at [M, N). Pair/validate only the common
                            # leading groups the prefill actually produced (the
                            # transfer path below likewise skips the trailing
                            # MTP groups). The full local_block_ids -- including
                            # the MTP groups -- is still stored so decode keeps
                            # its locally-allocated MTP blocks.
                            #
                            # GUARD: when group counts match (no MTP -- the
                            # proven EP8/EP8 256K path) n_common == every group
                            # and this is byte-identical to the old
                            # per-group "local <= remote" check.
                            n_common = min(
                                len(local_block_ids), len(remote_block_ids)
                            )
                            # Partial cache hits can lead to fewer local blocks vs
                            # remote blocks, but never more (checked per common
                            # group; plain zip truncates to the shorter list).
                            assert all(
                                len(local) <= len(remote)
                                for local, remote in zip(
                                    local_block_ids[:n_common],
                                    remote_block_ids[:n_common],
                                )
                            )
                        else:
...
class MoRIIOConnectorWorker:
    ...
    def __init__(
        ...
        # GLM53_PERGROUP_REMOTE_BLOCKS: per-layer remote block counts (see register_kv_caches).
        ...
        self.side_channel_port: int = (
            ...
            + get_port_offset(self.dp_rank, self.tp_rank, get_tensor_model_parallel_world_size())
        )
        self.engine_id: EngineId = engine_id
    ...
    def register_kv_caches(self, kv_caches: dict[str, torch.Tensor]):
        ...
        for layer_name in kv_caches:
            ...
            if hma_enabled:
                # GLM53_HYBRID_KV_NONUNIFORM: hybrid models (GLM-5.3-Flash: Mamba/KDA
                # linear-attention state layers interleaved with MLA) carry
                # legitimately different per-layer block lengths (state page
                # vs latent page). Transfer offsets are per-layer, so a
                # non-uniform block_len is correct; the advertised scalar
                # self.block_len is metadata only. Record per-layer instead
            ...
            # GLM53_HYBRID_KV_NONUNIFORM: num_blocks is reused as remote_num_blocks ONLY by
            # the split-KV dense path (transfers_per_block == 2). Single-region
            # layers (MLA latent, DSA sparse-indexer k_cache, Mamba/KDA state)
            # never consume it, and hybrid models legitimately give each KV
            # group a different block count -> only enforce for split-KV.
            ...
            # GLM53_PERGROUP_REMOTE_BLOCKS: remember each layer's own remote block count so
            # the consumer sets the correct per-group K/V split stride.
    ...
    def _compute_block_transfer_offsets(
        ...
        local, remote, sizes = compute_block_transfer_offsets(
            ...
            local_block_ids=local_block_ids,
            remote_block_ids=remote_block_ids,
            # GLM53_PERGROUP_REMOTE_BLOCKS: use this layer's own remote block count, not the
            # scalar first-layer count (wrong K/V split stride for hybrids).
            ...
            # GLM53_INDEXER_KBPB: reference GROUP-block counts (this rank's and the
            # remote rank's first-layer/scalar num_blocks). The DSA sparse-indexer
            # k_cache is paged at a finer kernel block than the group-block unit
            # block-ids are assigned in, so its per-layer num_blocks is an integer
            # multiple (kbpb) of these references. compute_block_transfer_offsets
            # uses that ratio to expand each group block-id into its kbpb kernel
            # sub-blocks. For all 1:1 layers kbpb==1 -> byte-identical old path.
    ...
    def _read_blocks(
        ...
        for layer_name in self.layer_name_to_local_kv_cache_metadata:
            ...
            # GLM53_MTP_DISAGG_RECONCILE (transfer side)
            # With MTP the decode leg carries EXTRA trailing KV group(s) for the
            # Glm5NextMTPModel draft layer that the prefill leg never computed,
            # so remote_block_ids has fewer groups than the local layer set. The
            # MTP group's KV is generated LOCALLY during decode -- there is
            # nothing to pull for it -- so skip any layer whose group index has
            # no corresponding remote (prefill) group. Base groups occupy the
            # same leading indices on both legs, so this only ever drops the
            # MTP-only trailing group(s). GUARD: no-MTP -> every group_idx is in
            # range -> byte-identical to the prior unconditional transfer.
            if group_idx >= len(remote_block_ids):
                continue
