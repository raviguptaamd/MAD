...
class MoRIIOWriter:
    ...
    def ensure_worker_started(self) -> None:
        ...
        # STREAMFIX: capture the HIP device on the MAIN thread (correct device/context)
        # so the daemon write-worker can set_device() to it. A fresh Python thread
        # defaults to device 0 with no active HIP context; any CUDA/mori op it runs
        # concurrently with the model forward corrupts the context -> the forward's
        # triton _init_handles / torch.mm hit 'invalid resource handle' / 'no kernel image'.
        try:
            import torch as _torch
            self._worker_cuda_device = _torch.cuda.current_device()
        except Exception:
            self._worker_cuda_device = None
    ...
    def schedule_write(self, task: WriteTask) -> bool:
        ...
        # MORIIO_SYNC_WRITES: run the write INLINE on the calling (main) thread instead of
        # handing it to the background 'moriio-write-worker' thread. mori's CreateSession /
        # BatchWrite touch HIP; on a fresh worker thread that corrupts the primary context so
        # the main thread's forward (cuBLAS torch.mm in the DSA indexer) hits 'invalid resource
        # handle' at real prompt lengths. Running the write on the main thread keeps ALL mori
        # HIP calls on the correct context. Safe because the deferred flush (wait_for_save) is
        # already on the main thread and the per-layer event.synchronize() ordering is preserved.
        import os as _os
        if _os.environ.get('MORIIO_SYNC_WRITES') == '1':
            try:
                self._execute_write_task(task)
            except Exception:
                logger.exception('Inline write task failed for %s', task.request_id)
    ...
    def _write_worker_loop(self) -> None:
        ...
        # STREAMFIX: pin this daemon thread to the main thread's HIP device so its
        # mori/CUDA work runs on the right device/context and never collides with the
        # model forward's triton JIT / kernel launches on the main thread.
        try:
            import torch as _torch
            _dev = getattr(self, '_worker_cuda_device', None)
            if _dev is not None:
    ...
    def _execute_write_task(self, task: WriteTask) -> None:
        ...
        try:
            ...
            print('[WRGUARD] _get_remote_alloc_info FAILED tid={}: {!r}'.format(task.transfer_id, _e), flush=True)
            _tb.print_exc()
            raise
        with self._write_state_lock:
        ...
        try:
            ...
            import traceback as _tb
            print('[WRGUARD] _prepare_transfer_plan FAILED layer={} tid={}: {!r}'.format(getattr(task,'layer_name','?'), task.transfer_id, _e), flush=True)
            _tb.print_exc()
            raise

        if not hasattr(self, "_wrdbg"): self._wrdbg=set()
        _gk = self.worker.layer_to_group_idx.get(task.layer_name, 0)
        if _gk not in self._wrdbg:
            self._wrdbg.add(_gk)
            try:
                print("[WRDBG] gidx={} layer={} local_off_n={} remote_off_n={} sizes_n={} sizes0={} bids={} agb={}".format(
                    _gk, task.layer_name,
                    len(plan.transfer_local_offsets), len(plan.transfer_remote_offsets),
                    len(plan.transfer_sizes), (plan.transfer_sizes[0] if plan.transfer_sizes else None),
                    request_info.block_ids,
                    ([len(g) for g in request_info.all_group_block_ids] if request_info.all_group_block_ids else None)), flush=True)
            except Exception as _e:
                print("[WRDBG] print failed: {!r}".format(_e), flush=True)
    ...
    def _prepare_transfer_plan(
        ...
        # Fold the KV-group index into the offset-cache key. Hybrid models
        # (GLM-5.3-Flash) have MULTIPLE mamba groups with IDENTICAL tensor
        # shape/stride/dtype but DISTINCT block ids; keying on geometry alone
        # made groups 3/4/5 reuse group 2's offsets -> wrote all mamba state
        # into one decode block, leaving the others stale -> garbage output.
        _gidx = getattr(
            self.worker, 'layer_to_group_idx', {}
        ).get(task.layer_name, 0)
        geometry_key = (_get_write_geometry_key(layer_cache), _gidx)
        offsets = request_info.transfer_offsets.get(geometry_key)
        if offsets is None:
            _remote_ids = request_info.block_ids
            _agb = request_info.all_group_block_ids
            if _agb is not None:
                if 0 <= _gidx < len(_agb):
                    _remote_ids = _agb[_gidx]
    ...
    def _do_layer_write(self, plan: LayerTransferPlan, sessions: list) -> list[Any]:
        ...
        """
        # SKIP_BATCHWRITE bisect gate: sessions are already BUILT (CreateSession ran in
        # _get_built_session); returning here executes everything EXCEPT the mori BatchWrite.
        # If long-ctx SURVIVES with this set -> BatchWrite is the HIP-ctx killer. If it still
        # crashes -> CreateSession (session build) is the killer. Diagnostic only.
        import os as _os
        if _os.environ.get("MORIIO_SKIP_BATCHWRITE") == "1":
            return []
        if plan.use_batch:
            return [
