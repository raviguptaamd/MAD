...
class BuildPrefillChunkMetadataKernel(
    ...
    def __call__(
        ...
        *,
        ...
        # IDXCTXFIX: under MoRIIO disagg, the KV-transfer path (mori CreateSession /
        # RegisterMemory / BatchWrite) mutates the HIP primary context, so Triton's
        # cached module handle for THIS chunk-metadata kernel is bound to a dead ctx ->
        # load_binary fails HIP-209 "no kernel image" on the multi-chunk prefill path
        # (colocated has no mori -> no crash). Re-assert the primary ctx AND drop this
        # kernel's in-memory device_caches immediately before the launch so _init_handles
        # reloads the .so into the LIVE ctx. Cheap: only the in-process handle re-creates.
        import os as _os
        if _os.environ.get("IDX_CTX_FIX", "1") == "1":
            try:
                import torch as _torch
                if _torch.cuda.is_available():
                    _torch.cuda.set_device(_torch.cuda.current_device())
                _dc = getattr(self.kernel, "device_caches", None)
                if _dc:
                    _dc.clear()
            except Exception:
                pass
