...
class Indexer(nn.Module):
    ...
    def forward(
        ...
        k = kw[:, : self.head_dim]
        # NO-CACHE head-gate: rebuild the fp32 weight slice from the PERSISTENT
        # model weight EVERY call instead of caching it across requests. Theory:
        # MoRIIO registers KV-cache GPU memory regions with RDMA; a tensor cached
        # in a prior (short) request lives in a pool whose handle goes stale after
        # the RDMA registration / a later multi-block forward -> torch.mm on the
        # cached tensor hits "invalid resource handle". self.wk_weights_proj.weight
        # is a persistent nn.Parameter (never RDMA-registered), so recomputing the
        # fp32 slice each call sidesteps the stale handle. The slice is tiny
        # (kv_lora x head_dim fp32) so the recompute cost is negligible.
        ...
        weights = _torch.mm(hidden_states.float(), _wp)
