...
_MORIIO_PAGE = 4096
def _page_aligned_zeros_int8(size: int, device):
    # Over-allocate one page and slice to a 4K-aligned base so ionic dma-buf
    # RDMA registration (which requires a page-aligned base) succeeds on the
    # DSA indexer KV tensors. The returned view is contiguous and used for both
    # compute and RDMA — no copy, no correctness change.
    backing = torch.zeros(size + _MORIIO_PAGE, dtype=torch.int8, device=device)
    off = (-backing.data_ptr()) % _MORIIO_PAGE
    view = backing[off:off + size]
    view._moriio_align_backing = backing  # keep backing alive
    return view
    ...
    for kv_cache_tensor in kv_cache_config.kv_cache_tensors:
        if kv_cache_tensor.block_stride > 0:
            ...
            if packed_backing is None:
                packed_backing = _page_aligned_zeros_int8(
                    kv_cache_tensor.size, device
                )
            tensor = packed_backing
        else:
            tensor = _page_aligned_zeros_int8(kv_cache_tensor.size, device)
