    ...
    def update_state_after_alloc(
        ...
        if params is not None and params.get("do_remote_prefill"):
            if self.mode == MoRIIOMode.READ:
                ...
                print('[DCDBG] WRITE-decode alloc ENTERED req={} num_ext={} kvp_keys={}'.format(
                    getattr(request, 'request_id', '?'), num_external_tokens,
                    list((request.kv_transfer_params or {}).keys())), flush=True)
                ...
                print('[DCDBG] should_notify={} req={}'.format(_should_notify, getattr(request, 'request_id', '?')), flush=True)
                if _should_notify:
                    ...
                    if os.environ.get("RECALL_PROBE") == "1":
                        try:
                            _gmap = getattr(self, "layer_to_group_idx", {})
                            from collections import Counter as _C2
                            _go = _C2()
                            for _l,_g in _gmap.items():
                                _go[_g]+=1
                            print("[NOTIFYDBG] req={} num_ext={} n_groups={} per_group_ids={} group_layercount={}".format(
                                request.request_id, num_external_tokens,
                                len(_notify_all_gids) if _notify_all_gids else 0,
                                [list(g) for g in _notify_all_gids] if _notify_all_gids else None,
                                dict(_go)), flush=True)
                        except Exception as _e:
                            print("[NOTIFYDBG] fail {!r}".format(_e), flush=True)
        ...
        if self.mode == MoRIIOMode.WRITE and get_role() == ROLE.PRODUCER:
            ...
            for i, req_id in enumerate(scheduler_output.scheduled_cached_reqs.req_ids):
                ...
                if new_block_ids is not None:
                    ...
                    # CHUNKFIX: extend the per-group hybrid block map across
                    # chunks so the final-chunk write covers EVERY chunk's
                    # blocks (MLA/indexer/mamba), not only chunk 1. Without
                    # this, _req_all_group_bids stays frozen at first-chunk
                    # alloc and multi-chunk prompts lose all but chunk-1 KV
                    # on decode (needle past ~one chunk missing at any depth).
                    _agb = getattr(self, "_req_all_group_bids", None)
                    if _agb is not None and req_id in _agb:
                        _cur = _agb[req_id]
                        for _g in range(min(len(_cur), len(new_block_ids))):
                            _ng = new_block_ids[_g]
                            if _ng:
                                _cur[_g] = list(_cur[_g]) + list(_ng)
                    if (
...
class MoRIIOConnectorWorker:
    ...
    def __init__(
        ...
        print("[PORTDBG] role={} ip={} local_kv_port={} handshake={} notify={}".format("P" if self.is_producer else "D", self.moriio_config.local_ip, self.moriio_config.local_kv_port, self.moriio_config.handshake_port, self.moriio_config.notify_port), flush=True)
    ...
    def _moriio_handshake_listener(
        ...
        print(
            "[LISTENDBG] handshake listener BIND base_port={} dp_rank={} "
            "tp_rank={}".format(base_port, dp_rank, tp_rank),
            flush=True,
        )
            ...
            while True:
                ...
                if (
                    ...
                    try:
                        ...
                        print("[HSDBG] frame2 building: nlayers={} bytes={}".format(len(layer_name_to_local_kv_cache_metadata), len(buf)), flush=True)
                        sock.send_multipart((identity, b"", buf))
                        print("[HSDBG] frame2 SENT ok", flush=True)
                    except Exception as _e:
                        import traceback
                        print("[HSDBG] frame2 FAILED: {!r}".format(_e), flush=True)
                        traceback.print_exc()
    ...
    def _moriio_handshake(
        ...
        # Port slot MUST match the remote listener's bind formula. The listener
        # (side_channel_port) binds at handshake_port + get_port_offset(dp, tp)
        # using get_port_offset's DEFAULT stride (4 ports/dp rank), independent
        # of the actual tp_size. Passing remote_tp_size here collapses the stride
        # to 1 when tp_size==1 (EP disagg), so every dp>0 dial hits the WRONG
        # port (dp1 -> base+1 instead of base+4) and the handshake hangs; dp0 is
        # offset 0 either way, which is why only dp0 worked. Use the same default
        # stride the listener used so dp0..N all dial the correct port. TP-prefill
        # mirror (remote_tp_size==4) is byte-identical (4 == the default).
        port_offset = get_port_offset(remote_dp_rank, dial_tp_rank)
        path = make_zmq_path("tcp", host, port + port_offset)
        logger.debug("handshake Querying metadata on path: %s", path)
        print(
            "[DIALDBG] dial engine={} host={} base_port={} remote_dp_rank={} "
            "dial_tp_rank={} remote_tp_size={} -> offset={} PATH={}".format(
                expected_engine_id, host, port, remote_dp_rank, dial_tp_rank,
                remote_tp_size, port_offset, path),
            flush=True,
        )
        ...
        with zmq_ctx(zmq.DEALER, path) as sock:
            ...
            print("[HSDBG-PF] waiting for frame2 (2nd recv) from {}:{}".format(host, port), flush=True)
            received_frame = sock.recv_multipart()
            print("[HSDBG-PF] frame2 RECEIVED: nparts={} len1={}".format(len(received_frame), (len(received_frame[1]) if len(received_frame)>1 else -1)), flush=True)
            if len(received_frame) != 2 or received_frame[0] != b"":
                raise HandshakeError(f"unexpected frame! {received_frame = }")
            ...
            print("[HSDBG-PF] frame2 parsed + stored OK for {}".format(expected_engine_id), flush=True)
    ...
    def _background_moriio_handshake(
        ...
        if fut is None:
            ...
            # EP disagg all-to-all handshake fix. The decode leg notifies from
            # WHICHEVER of its DP ranks served the request (decode_rank in the
            # notify; the write engine then looks up f"{engine}_dp{decode_rank}").
            # Every such rank must be handshaked HERE or _get_built_session
            # KeyErrors and the write never fires (60s stall -> router HTTP 000).
            # The vLLM router does not advertise the decode DP size in
            # kv_transfer_params, so meta.remote_dp_size defaults to 1 and only
            # dp0 was handshaked -> dp1..7 deadlock. This 1P1D deployment is
            # symmetric (decode DP == this prefill's DP), so when the peer size
            # was not propagated fall back to our own DP fan-out. Single-DP
            # (TP4) keeps 1 -> byte-identical to the working path.
            _self_dp = int(self.vllm_config.parallel_config.data_parallel_size)
            _self_dp_local = (
                int(self.vllm_config.parallel_config.data_parallel_size_local)
                or _self_dp
            )
            if remote_dp_size < _self_dp:
                logger.warning(
                    "MoRIIO handshake: remote_dp_size=%d < local dp=%d; "
                    "assuming symmetric EP disagg and handshaking all %d decode "
                    "DP ranks (was only dp0 -> dp1..N deadlock).",
                    remote_dp_size, _self_dp, _self_dp,
                )
                remote_dp_size = _self_dp
                remote_dp_size_local = _self_dp_local
            print(
                "[DPFIX] _background_handshake remote_engine={} remote_dp_size={} "
                "remote_dp_size_local={} tp_size={} base_port={} pod_hosts={}".format(
                    remote_engine_id, remote_dp_size, remote_dp_size_local,
                    tp_size, port, pod_hosts),
                flush=True,
            )
    ...
    def get_finished(self) -> tuple[set[str], set[str]]:
        ...
        if self.is_producer:
            ...
            if self.mode == MoRIIOMode.WRITE:
                ...
                if fresh and os.environ.get("NORM_PROBE") == "1":
                    try:
                        if not hasattr(self, "_norm_probe_done"):
                            self._norm_probe_done = True
                            import torch as _t
                            _rep = {}
                            for _ln, _kc in self.kv_caches.items():
                                _g = self.layer_to_group_idx.get(_ln, -1)
                                if _g not in _rep:
                                    _rep[_g] = (_ln, _kc)
                            for _g in sorted(_rep):
                                _ln, _kc = _rep[_g]
                                _sp = type(self.layer_to_spec.get(_ln)).__name__
                                _norms = []
                                for _b in range(0, min(9, _kc.shape[0])):
                                    try:
                                        _norms.append(round(float(_kc[_b].float().norm().item()), 3))
                                    except Exception:
                                        _norms.append(-1.0)
                                print("[NORMPROBE] g={} spec={} layer={} block_norms[0:9]={}".format(
                                    _g, _sp, _ln.split(".")[-2] if "." in _ln else _ln, _norms), flush=True)
                    except Exception as _e:
                        print("[NORMPROBE] fail {!r}".format(_e), flush=True)
            else:
            ...
            if _os.environ.get("MORIIO_EAGER_SESSIONS", "1") == "1":
                try:
                    ...
                    if _t.cuda.is_available():
                        ...
                        _t.cuda.synchronize()
                    print("[EAGERSESS] pre-built write sessions for {}".format(remote_engine_id), flush=True)
                except Exception as _e:
                    print("[EAGERSESS] pre-build failed for {}: {!r}".format(remote_engine_id, _e), flush=True)
    ...
    def _write_blocks_for_req(self, req_id: ReqId, meta: ReqMeta, layer_name, kv_layer):
        ...
        if not hasattr(self, "_dbg_wr"): self._dbg_wr=set()
        _spn = type(self.layer_to_spec.get(layer_name)).__name__
        if "__hist__" not in self._dbg_wr:
            self._dbg_wr.add("__hist__")
            from collections import Counter as _C
            _h=_C()
            for _l,_g in self.layer_to_group_idx.items():
                _h[(_g, type(self.layer_to_spec.get(_l)).__name__)]+=1
            print("[WRHIST] groups(idx,spec)->nlayers = {}".format(dict(_h)), flush=True)
            if _agl is not None:
                print("[WRHIST] agl group sizes = {}".format([len(g) for g in _agl]), flush=True)
        if "__full__" not in self._dbg_wr:
            self._dbg_wr.add("__full__")
            print("[WRFULL] all_group_local_block_ids = {}".format(_agl), flush=True)
            print("[WRFULL] meta.local_block_ids(group0) = {} remote_block_ids = {}".format(
                meta.local_block_ids, meta.remote_block_ids), flush=True)
