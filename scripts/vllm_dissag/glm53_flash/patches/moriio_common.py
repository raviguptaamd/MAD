...
class RemoteAllocInfo:
    ...
    block_ids: list[int]
    # Per-KV-group REMOTE (decode) block ids for hybrid models. None =>
    # use block_ids for every layer (byte-identical old path).
    all_group_block_ids: Any = None
    writes_done: int = 0
...
def get_port_offset(dp_rank: int, tp_rank: int, tp_size: int = 4) -> int:
...
class ReqMeta:
    ...
    # Per-KV-group LOCAL (producer) block ids for hybrid models. Index =
    # group idx in kv_cache_config.kv_cache_groups (same order as
    # get_block_ids()). None => single group; all layers use
    # local_block_ids (byte-identical old path).
    all_group_local_block_ids: Any = None
...
class MoRIIOConnectorMetadata(KVConnectorMetadata):
    ...
    def add_new_req(
        ...
        write_mode=False,
        all_group_local_block_ids=None,
    ):
        ...
        _req = ReqMeta(
            ...
            all_group_local_block_ids=all_group_local_block_ids,
        )
        if write_mode:
