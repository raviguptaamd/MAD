...
def batched_gemm_a16wfp4_(
    ...
    import os as _os
    if _os.environ.get("GEMMDBG")=="1":
        print("[GEMMDBG] B={} M={} N={} K={} xstr={} wstr={} cfg={}".format(B,M,N,K,tuple(x.stride()),tuple(w.stride()),{k:config.get(k) for k in ["BLOCK_SIZE_M","BLOCK_SIZE_N","BLOCK_SIZE_K","NUM_KSPLIT","matrix_instr_nonkdim","kpack","num_warps","num_stages","waves_per_eu"] if k in config}), flush=True)
